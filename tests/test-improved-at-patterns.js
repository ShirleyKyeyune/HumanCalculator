...
function testPattern(input) {
  ...
  // Improved pattern for "300 bottles @ 1.2k" format
  ...
  // If no pattern matched, try the pattern without space after @
  if (!matched) {
    result = input.replace(/(\d+(?:,\d+)?)\s*(?:bottles|bottle)\s*@(\d+(?:,\d+)?|\d+\.\d+k)/i, (match, num1, num2) => {
      console.log(`  No-space pattern matched: num1=${num1}, num2=${num2}`);
      matched = true;
      // Remove commas and handle 'k' suffix
      const cleanNum1 = num1.replace(/,/g, '');
      let cleanNum2 = num2.replace(/,/g, '');
